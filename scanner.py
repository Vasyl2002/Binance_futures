...
ACCUM_P24H_MIN = -30.0       # p24h не в глубоком дампе
ACCUM_P24H_MAX = 100.0       # до +100% — не отсекаем по p24h (ALICE/BOB и т.д.)
...
STEALTH_MIN_SCORE = 35       # 35+ даём алерт; иначе ноль алертов
...
STEALTH_IMPROVE_MIN = 2.0    # повтор при росте score на +2 (было 4 — резало повторы)
...
LS_CALL_LONG_RATIO_MIN = 1.50   # L/S > 1.5 = 60%+ longs (если включить)
LS_CALL_SHORT_RATIO_MAX = 0.70  # L/S < 0.7 = 58%+ shorts
LS_CALL_TOP_N = 2               # макс алертов за скан (если включить)
LS_CALL_P24H_MAX_LONG = 15.0
LS_CALL_P24H_MIN_SHORT = -15.0
LS_CALL_FUNDING_FILTER = True
LS_CALL_COOLDOWN_SEC = 900      # 15 мин
LS_CALL_INTERVAL_SEC = 300
LS_CALL_MIN_Q24H = 5_000_000.0
    ...
    async def _accumulation_loop(self):
        ...
        while True:
            try:
                ...
                all_scored: List[Tuple[float, str]] = []  # (score, sym) для диагностики при passed=0
                ...
                if checked > 0 or len(candidates) > 0:
                    ...
                    if passed == 0 and all_scored:
                        top3 = sorted(all_scored, key=lambda x: x[0], reverse=True)[:3]
                        print(f"[ACCUM] passed=0 → top3 by score: {', '.join(f'{s}({sc:.0f})' for sc, s in top3)}")
    ...
    async def _ls_call_loop(self):
        ...
        while True:
            try:
                ...
                checked = 0
                ls_signals: List[Tuple[float, str, str, float, Optional[float]]] = []  # (score, sym, side, ratio, funding)
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20)) as http:
                    for sym in symbols[:80]:
                        ...
                        score = ratio if side == "LONG" else (1.0 - ratio)
                        ls_signals.append((score, sym, side, ratio, funding_pct))

                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as http_tg:
                    for score, sym, side, ratio, funding_pct in sorted(ls_signals, key=lambda x: x[0], reverse=True)[:LS_CALL_TOP_N]:
                        st = STATES.get(sym)
                        if st is not None:
                            st.last_ls_call_ts = now
                            st.last_ls_call_side = side
                        p24h_v = float(getattr(st, "p24h", 0) or 0) if st else 0.0
                        fund_str = f" fund={funding_pct*100:.4f}%" if funding_pct is not None else ""
                        msg = f"{sym} LS_CALL ({side}) | L/S {ratio:.2f} | p24h {p24h_v:.1f}%{fund_str}"
                        print(msg)
                        if TG_STRUCTURED:
                            extra = [f"L/S {ratio:.2f} · p24h {p24h_v:.1f}%"]
                        ...
                        if self.tg_enabled and self.tg_rl.allow():
                            try:
                                ...
                                pass
                if checked > 0:
                    print(f"[LS_CALL] scan done, checked={checked} syms, sent={min(len(ls_signals), LS_CALL_TOP_N)}")
            except Exception as e:
